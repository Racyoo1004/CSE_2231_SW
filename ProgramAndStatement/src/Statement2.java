...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        StatementLabel rt = new StatementLabel(Kind.BLOCK);
        Sequence<Tree<StatementLabel>> sub = this.rep.newSequenceOfTree();
        this.rep.assemble(rt, sub);
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Sequence<Tree<StatementLabel>> sub = this.rep.newSequenceOfTree();
        StatementLabel rt = this.rep.disassemble(sub);
        sub.add(pos, localS.rep);
        this.rep.assemble(rt, sub);
        localS.createNewRep();
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        Sequence<Tree<StatementLabel>> sub = this.rep.newSequenceOfTree();
        StatementLabel rt = this.rep.disassemble(sub);
        s.rep.transferFrom(sub.remove(pos));
        this.rep.assemble(rt, sub);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> sub = this.rep.newSequenceOfTree();
        sub.add(0, localS1.rep);
        sub.add(1, localS2.rep);
        this.rep.assemble(label, sub);
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Statement2 localS2 = (Statement2) s2;
        Sequence<Tree<StatementLabel>> sub = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(sub);
        localS2.rep = sub.remove(1);
        localS1.rep = sub.remove(0);
        this.createNewRep();
        return label.condition;
    }
